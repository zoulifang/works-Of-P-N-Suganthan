...
for fun = 1:30
    for run=1:1
        ...
        mixVal = (cec14_func(mixPop', fun))';%benchmark_func(mixPop, fun, o, A, M, a, alpha, b);
%         valOffspringxx = cec14_func(ui', fun);
        ...
        valxx  = cec14_func(FM_pop(I_best_index, :)',fun);%benchmark_func(FM_pop(I_best_index, :), fun, o, A, M, a, alpha, b);
        val(1) =valxx';
        F_bestval = val(1);     % best objective function value so far
        FES = FES + 1;
        for k = 2 : I_NP       % check the remaining members
            valxx = cec14_func(FM_pop(k, :)',fun);%benchmark_func(FM_pop(k, :), fun, o, A, M, a, alpha, b);
            val(k) = valxx';
            FES = FES + 1;
            if (val(k) < F_bestval)
...
%             valOffspring = benchmark_func(ui, fun, o, A, M, a, alpha, b);
            valOffspringxx = cec14_func(ui', fun);%benchmark_func(ui, fun, o, A, M, a, alpha, b);
...
%             fitSet = benchmark_func(uSet, fun, o, A, M, a, alpha, b);
            fitSetxx = cec14_func(uSet', fun); %benchmark_func(uSet, fun, o, A, M, a, alpha, b);
            fitSet = fitSetxx';
            for i = 1 : popsizeC
...
%             tempval = benchmark_func(FM_ui, fun, o, A, M, a, alpha, b);
            tempvalxx = cec14_func(FM_ui', fun);
            tempval = tempvalxx';
